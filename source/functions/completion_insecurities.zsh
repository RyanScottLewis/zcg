...
function handle_completion_insecurities() {
  ...
  if (( ! ${#insecure_dirs} )); then
      print "No insecure completion-dependent directories detected."
      return
  fi
  ...
  ls -ld "${(@)insecure_dirs}"
  print "For safety, completions will be disabled until you manually fix all"
  print "Insecure directory permissions and ownership and restart oh-my-zsh."
  print "See the above list for directories with group or other writability.\n"
  ...
  if (( ${#zcompdump_files} )); then
    ...
    print "Insecure completion caches also detected:"
    ls -l "${(@)zcompdump_files}"

    # For safety, move rather than permanently remove such files.
    print "Moving to \"${ZSH_ZCOMPDUMP_BAD_DIR}/\"...\n"
